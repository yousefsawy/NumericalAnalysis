...
def max_deflation_matrix(matrix1, vector, lamda_max):
    vector_norm_sq = np.linalg.norm(vector)**2
    vector_t = np.transpose(vector)
    result_matrix = [[element * lamda_max / vector_norm_sq] for element in vector]
    max_deflation_matrix = matrix1 - np.outer(result_matrix, vector_t)
    return max_deflation_matrix

#Get the minimum deflation matrix
def min_deflation_matrix(matrix1, vector,lamda_min):
    vector_norm_sq = np.linalg.norm(vector)**2
    vector_t = np.transpose(vector)
    matrix1_inv=np.linalg.inv(matrix1)
    result_matrix=[[element*lamda_min/vector_norm_sq] for element in vector]
    min_deflation_matrix = matrix1_inv - np.outer(result_matrix, vector_t)
