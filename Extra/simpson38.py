...
def simpsons_3_8_rule(f, a, b, n, mode):
    ...
    if mode == 1:
        z = sp.symbols('x')
        third_derivative = sp.diff(f(z), z, 3)
        fourth_derivative = (third_derivative.subs(z, x[-1]) - third_derivative.subs(z, x[0]))/(x[-1] - x[0])
        error = ((x[-1]-x[0])*5 *fourth_derivative)/(6840*n*4)  # Error calculation using scipy's quad function
    else:
        error = 0
...
def main():
    ...
    if user_choice == 1:
        ...
        if n % 3 != 0 or n <= 0:
            print("Invalid Number of Intervals, enter a positive number divisible by 3")
            
        else :
            func = convert_to_function(fstring)
